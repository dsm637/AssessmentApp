...
namespace ConsoleApp6
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var apiClient = new NpsClient(client);

            var nearestParks = apiClient.GetParks("Waltham", "MA", CancellationToken.None).Result;
            foreach( var park in nearestParks )
            {
                Console.WriteLine($"{park.Name}");
                Console.WriteLine($"{park.Url}");
